...
%     end
    ...
    sheetMatrix = zeros( (maxNote - minNote +1 ) * basicParameter.rankMode + 1, specLength);
...
%         attMargin = basicParameter.attackLengthFrame - 1;  
        ...
        elseif basicParameter.rankMode > 3
            for j=1:basicParameter.rankMode
                tempBasisIndex = (basisIndex-2) * basicParameter.rankMode +1 + j;
                tempOnset = onset  + basicParameter.multiRankHopFrame * (j-1);
                if tempOnset > offset
                    break
                end
